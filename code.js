...
function getDifficultPasswordForUser(username, difficulty) {
  ...
  let hashedUserName = sha1(DIFFICULT_PASSWORD_SALT + difficulty + username);

  // Convert to an a-z string

  let hashedUserNameAsString = bigInt(hashedUserName, 16).toString(
    26,
...
function getPasswordForUser(username) {
  ...
  let hashedUsername = sha1(username);

  // Use as an index into a secret list of passwords

  let listIndex =
    parseInt(hashedUsername.slice(0, 4), 16) % secretListOfPasswords.length;
...
async function stall(time, difficulty) {
  await msleep(time / difficulty + randn_bm() * difficulty);
}

async function verifyPassword(inPassword, correctPassword, difficulty) {
  if (inPassword.length != correctPassword.length) {
    return false;
  }

  await stall(BASE_TIME_TO_STALL, difficulty);
  ...
  for (let i = 0; i < correctPassword.length; i++) {
    ...
    if (inPassword[i] != correctPassword[i]) {
      ...
      for (let j = i; j < correctPassword.length; j++) {
        await stall(0, difficulty);
      }
...
app.get("/", async function (req, res) {
  ...
  if (req.query.user != null && req.query.password != null) {
    ...
    if (req.query.difficulty != null) {
      ...
      secretPassword = getDifficultPasswordForUser(
        req.query.user,
    ...
    // we didn't get both user and password

    res
      .status(200)
      .send(
        "Usage: http://aoi-assignment1-async.oy.ne.ro/?user=albert&password=perot[&difficulty=1]"
      );
  }
});
